module Shipatron4000
    ...
    class << self
        ...
        def tier cart
            ...
            return Tier.where('? >= length_start AND ? <= length_end AND ? >= thickness_start AND ? <= thickness_end AND ? >= weight_start AND ? <= weight_end', max_length, max_length, max_thickness, max_thickness, total_weight, total_weight)
        end
    end
end