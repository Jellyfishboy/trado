...
class Cart < ActiveRecord::Base
  ...
  # scope :find_sku, -> { where('sku_id = ?',sku.id).includes(:cart_item_accessory) }

  # current_item = accessory.blank? ? cart_items.find_sku.where(:cart_item_accessories => { :accessory_id => nil }).first : cart_items.find_sku.where(:cart_item_accessories => { :accessory_id => accessory.id }).first

  # # if cart item exists and no accessory value is blank and cart item accessory is nil
  # # OR
  # # if cart item exists and accessory value and cart item accessory exists
