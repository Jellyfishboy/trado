class Cart < ActiveRecord::Base
  ...
  def add_product(product_id, product_price, dimension_id, dimension_length, dimension_thickness, dimension_weight, product_sku)
    ...
    if current_item
      ...
      current_item.weight += dimension_weight
  		...
  		current_item = line_items.build(:product_id => product_id, :price => product_price, :dimension_id => dimension_id, :length => dimension_length, :thickness => dimension_thickness, :weight => dimension_weight, :sku => "#{product_sku}-#{dimension_length.floor}") #if line item selected does not exist, build a new cart item
  	end
